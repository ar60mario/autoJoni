...
public class FacturaWeb2Frame extends javax.swing.JFrame {

    public List<RenglonFactura> renglonFactura = new ArrayList<>();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FacturaWeb2Frame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FacturaWeb2Frame().setVisible(true);
            }
        });
    }
        ...
        if (tst == 0) {
            try {
                ...
                Dispatch.call(wsaa, "Autenticar",
                        new Variant("wsfe"),
                        new Variant(userdir + Constantes.certif),
                        new Variant(userdir + Constantes.llave),
                        new Variant(wsdl));
                ...
                Dispatch.put(wsfev1, "Cuit", new Variant(Constantes.cuitTitular));
                ...
                numCae = cae.toString();
                String ruta1 = Constantes.ruta + tipoComprob
                        + letraFacturaPapel + sucursalFacturaPapel
                        + numeroFacturaPapel + ".xm1";
                String ruta2 = Constantes.ruta + tipoComprob
                        + letraFacturaPapel + sucursalFacturaPapel
...
//            }
    ...
    private void pdf(IvaVentas iv, List<RenglonFactura> rf) {
        Date fechaComp = iv.getFecha();
        String cuitT = Constantes.cuitTitular;
        String ptoVta = iv.getNumeroSucursal().toString();
        Integer nroCpbte=iv.getNumeroFactura();
        String tipoComp=iv.getTipoDoc().toString();
        Double totalFc = iv.getTotal();
        String tipoDocumReceptor=iv.getCliente().getTipo();
        String nroDocumReceptor = iv.getCliente().getCuit();
        String caeNro = iv.getCae().toString();
        String nroComprStr = dfn.format(nroCpbte);
        String data = UtilQr.crearDataQr(fechaComp, cuitT, ptoVta, nroCpbte, 
                tipoComp, totalFc, tipoDocumReceptor, nroDocumReceptor, caeNro);
        try {
            UtilQr.generarPngQr(data, nroComprStr);
        } catch (Exception ex) {
            Logger.getLogger(FacturaWeb2Frame.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(this, "ERROR CREAR QR");
            return;
        }
